...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(mqtt_client_id, mqtt_user, mqtt_password)) {
      ...
      client.subscribe("commands/fr1/fd1");
      client.subscribe("commands/fr1/fd2");
    } else {
...
void loop() {
  ...
  if (shouldPublish1 && !isnan(temp1) && !isnan(hum1) && !isnan(light1)) {
    publishSensorData("fr1", "fd1", temp1, hum1, soil1Percent, light1);
  ...
  int soil2Percent = map(analogRead(SOIL2_PIN), 4095, 0, 0, 100);
  ...
  if (shouldPublish2 && !isnan(temp2) && !isnan(hum2) && !isnan(light2)) {
    publishSensorData("fr1", "fd2", temp2, hum2, soil2Percent, light2);
    lastLight2 = light2; lastTemp2 = temp2; lastHum2 = hum2; lastSoil2 = soil2Percent;
  }
